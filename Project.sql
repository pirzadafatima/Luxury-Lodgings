create database luxuryLodgings
use luxuryLodgings
...
--Trigger for FK constraint on bill table to avoid error of multiple cascading paths 
go
...
go
alter TRIGGER trg_bookingID_Cars
ON [Car Booking]
AFTER INSERT, UPDATE 
AS
BEGIN
...
BEGIN
	...
	if(@invoice IS not null) 
		...
		begin
			INSERT INTO Bill (Guest_ID, Booking_ID, RoomCharge, TotalBill) VALUES (@gID, @bID, @d, 0)
		end
END 
go
...
AS
SELECT [Room Booking].Guest_ID As Guest_ID, Rooms.Type As [Room Type], Rooms.Price As [Room Price], Rooms.NoOfAdults [Room Adult Accomodation], Rooms.NoOfChildren As [Room Child Accomodation], [Room Booking].Booking_Date As [Room Booking Date], [Room Booking].Check_In As [Room Check In DateTime], [Room Booking].Check_Out As [Room Check Out DateTime], [Room Booking].Price As [Room Booking Price],[Room Booking].SpecialRequest As [Room Booking Special Request] FROM Rooms Join [Room Booking] on Rooms.Room_ID = [Room Booking].Room_ID
go
